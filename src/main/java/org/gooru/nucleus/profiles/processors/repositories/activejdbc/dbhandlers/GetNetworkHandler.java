...
public class GetNetworkHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (details.equalsIgnoreCase(HelperConstants.REQ_PARAM_DETAILS_FOLLOWERS)) {
      ...
      followers.stream().forEach(follower -> detailsArray.add(new JsonObject(new JsonFormatterBuilder()
              .buildSimpleJsonFormatter(false, AJEntityUserDemographic.DEMOGRAPHIC_FIELDS).toJson(userDemographicsMap.get(follower.toString())))
      ...
      followings.stream().forEach(follower -> detailsArray.add(new JsonObject(new JsonFormatterBuilder()
              .buildSimpleJsonFormatter(false, AJEntityUserDemographic.DEMOGRAPHIC_FIELDS).toJson(userDemographicsMap.get(follower.toString())))
                      .put(AJEntityUserNetwork.FOLLOWERS_COUNT, followersCountMap.get(follower.toString()))
                      .put(AJEntityUserNetwork.FOLLOWINGS_COUNT, followingsCountMap.get(follower.toString()))));
    }
  ...
  private Map<String, AJEntityUserDemographic> getDemographics(List input) {
    LazyList<AJEntityUserDemographic> userDemographics =
            AJEntityUserDemographic.findBySQL(AJEntityUserDemographic.SELECT_DEMOGRAPHICS_MULTIPLE, listToPostgresArrayString(input));
  ...
  private Map<String, Integer> getFollowersCount(List input) {
    List<Map> followersCount = Base.findAll(AJEntityUserNetwork.SELECT_FOLLOWERS_COUNT_MULTIPLE, listToPostgresArrayString(input));
  ...
  private Map<String, Integer> getFollowingsCount(List input) {
    List<Map> followingsCount = Base.findAll(AJEntityUserNetwork.SELECT_FOLLOWINGS_COUNT_MULTIPLE, listToPostgresArrayString(input));
    Map<String, Integer> followingsCountMap = new HashMap<>();
    followingsCount.stream().forEach(map -> followingsCountMap.put(map.get(AJEntityUserNetwork.FOLLOW_ON_USER_ID).toString(),
