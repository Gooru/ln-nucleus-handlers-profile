...
public class SearchCollectionHandler implements DBHandler {

  private final ProcessorContext context;
  private static final Logger LOGGER = LoggerFactory.getLogger(SearchCollectionHandler.class);
  ...
  public SearchCollectionHandler(ProcessorContext context) {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (isPublic) {
      selectQuery = AJEntityCollection.SEARCH_COLLECTIONS_FOR_PUBLIC;
    } else {
      selectQuery = AJEntityCollection.SEARCH_COLLECTIONS_FOR_OWNER;
    }
    ...
    if (searchText != null ) {
      selectQuery = selectQuery + AJEntityCollection.OP_AND + AJEntityCollection.CRITERIA_TITLE;
    ...
    LazyList<AJEntityCollection> collectionList = AJEntityCollection.findBySQL(selectQuery, params.toArray());
    JsonObject responseBody = new JsonObject();
    responseBody.put("collections", new JsonArray(new JsonFormatterBuilder().buildSimpleJsonFormatter(false, AJEntityCollection.COLLECTION_LIST).toJson(collectionList)));
